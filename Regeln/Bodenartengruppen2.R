...
#######################################################################################################
#######################################################################################################
print("Zusammenfassung der schichtweisen Bodenartengruppenkategorien zu einer Zeichenfolge")
#######################################################################################################
#######################################################################################################
#######################################################################################################
#######################################################################################################
...
#  setwd(file.path(W.DIR,RESULT.DIR))
#  pdf(paste(substr(RU,1,nchar(RU)-4),"_BAG_Flaechenanteile",sep="",".pdf"), height=5,width=5)
...
r@data$BAG_STB  <- paste(c$STB_1,
                        ...
                        sep="")
#eliminating no changes
r@data$BAG_STB <- gsub("NA", "", r@data$BAG_STB)
head(r@data$BAG_STB,100)
#######################################################################################################
...
r@data$BAG_MS1  <- paste(c$MS1_1,
                        ...
                        sep="")
#eliminating no changes
r@data$BAG_MS1 <- gsub("NA", "", r@data$BAG_MS1)
#######################################################################################################
...
r@data$BAG_SRC  <- paste(c$SRC_1,
                    ...
                    sep="")
#eliminating no changes
r@data$BAG_SRC <- gsub("NA", "", r@data$BAG_SRC)
head(r@data$BAG_SRC,100)
#######################################################################################################
#export
#######################################################################################################
setwd(file.path(W.DIR,RESULT.DIR))
shapefile(r, paste(substr(RU,1,nchar(RU)-4),"_BAG_TYP",sep="",".shp"), overwrite=TRUE)
df.tx <- data.frame(ID=r@data$ID,r@data[grepl("BAG", names(r@data))])
setwd(file.path(W.DIR,RESULT.DIR))
write.table(df.tx, file = paste(substr(RU,1,nchar(RU)-4),"_BAG_TYP",sep="",".csv"), 
            append = FALSE, 
            quote = FALSE, 
            sep = ";",
            dec = ",", 
