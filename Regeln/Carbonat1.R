...
#------------------------------------------------------------------------------------------------------
setwd((paste(W.DIR,RULE.DIR,"Carbonat/",sep="")))
#classification
cal.lt <- list.files(pattern="(^c[0-6]{1})_.*\\.R$")
for(j in cal.lt) {
...
write.table(s, file = paste(substr(S.L[L],1,nchar(S.L[L])-4),"_CAR",sep="",".csv"), 
            append = FALSE, 
            quote = FALSE, 
            sep = ";",
            dec = ",", 
