...
#pdf(paste(substr(RU,1,nchar(RU)-4),"_SKE_Flaechenanteile",sep="",".pdf"), height=5,width=5)
...
#                xlab=paste("Skelettgehaltsstufen | ",groupColumns),
#                las=2,
#                ylim=c(0,100))
...
r@data$SKE_STB  <- paste(c$STB_1,
                        ...
                        sep="")
#eliminating no changes
r@data$SKE_STB <- gsub("NA", "", r@data$SKE_STB)
head(r@data$SKE_STB,100)
#######################################################################################################
...
r@data$SKE_MS1  <- paste(c$MS1_1,
                        ...
                        sep="")
#eliminating no changes
r@data$SKE_MS1 <- gsub("NA", "", r@data$SKE_MS1)
#######################################################################################################
...
r@data$SKE_SRC  <- paste(c$SRC_1,
                        ...
                        sep="")
#eliminating no changes
r@data$SKE_SRC <- gsub("NA", "", r@data$SKE_SRC)
#######################################################################################################
#export
#######################################################################################################
setwd(file.path(W.DIR,RESULT.DIR))
head(r)
shapefile(r, paste(substr(RU,1,nchar(RU)-4),"_SKE_TYP",sep="",".shp"), overwrite=TRUE)
df.ske <- data.frame(ID=r@data$ID,r@data[grepl("SKE", names(r@data))])
setwd(file.path(W.DIR,RESULT.DIR))
write.table(df.ske, file = paste(substr(RU,1,nchar(RU)-4),"_SKE_TYP",sep="",".csv"), 
            append = FALSE, 
            quote = FALSE, 
            sep = ";",
            dec = ",", 
