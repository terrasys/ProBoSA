...
#######################################################################################################
#######################################################################################################
print("Zusammenfassung der schichtweisen Carbonatsgehaltsstufen zu einer Zeichenfolge")
#######################################################################################################
#######################################################################################################
#######################################################################################################
#######################################################################################################
...
#  r@data[which(r@data[paste("MS1_",k,sep="")]==0),][paste("CLASS_",k,sep="")] <-  "??"
#}
#####################################################################################################
...
#pdf(paste(substr(RU,1,nchar(RU)-4),"_CAR_Flaechenanteile",sep="",".pdf"), height=5,width=5)
...
#                xlab=paste("Carbonatgehaltsstufen | ",groupColumns),
#                las=2,
#                ylim=c(0,100))
...
r@data$CAR_STB  <- paste(c$STB_1,
                        ...
                        sep="")
#eliminating no changes
r@data$CAR_STB <- gsub("NA", "", r@data$CAR_STB)
#######################################################################################################
...
r@data$CAR_MS1  <- paste(c$MS1_1,
                        ...
                        sep="")
#eliminating no changes
r@data$CAR_MS1 <- gsub("NA", "", r@data$CAR_MS1)
#######################################################################################################
...
r@data$CAR_SRC  <- paste(c$SRC_1,
                        ...
                        sep="")
#eliminating no changes
r@data$CAR_SRC <- gsub("NA", "", r@data$CAR_SRC)
head(r@data$CAR_SRC,100)
#######################################################################################################
#export
#######################################################################################################
setwd(file.path(W.DIR,RESULT.DIR))
head(r)
shapefile(r, paste(substr(RU,1,nchar(RU)-4),"_CAR_TYP",sep="",".shp"), overwrite=TRUE)
df.car <- data.frame(ID=r@data$ID,r@data[grepl("CAR", names(r@data))])
setwd(file.path(W.DIR,RESULT.DIR))
write.table(df.car, file = paste(substr(RU,1,nchar(RU)-4),"_CAR_TYP",sep="",".csv"), 
            append = FALSE, 
            quote = FALSE, 
            sep = ";",
            dec = ",", 
